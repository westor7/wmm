...
dialog -l wus_sets {
  ...
  check "Display the module output messages", 620, 4 85 150 10, tab 13
...
ON *:DIALOG:wus_sets:*:*: {
  if ($devent == init) {
    ...
    if ($istok(% [ $+ [ $mod ] $+ ] _show,format,32)) { did -c $dname 31 }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,provider,32)) { did -c $dname 32 }
  ...
  if ($devent == close) {
    .timer[ $+ $mod $+ _*] off
    if (!% [ $+ [ $mod ] $+ ] _show) { set % $+ $mod $+ _show title provider }
...
ON *:LOAD: { 
  ...
  if (% [ $+ [ $mod ] $+ ] _show == $null) { set % $+ $mod $+ _show title description provider format }
  hfree -w $mod $+ _*
  .signal -n wmm_close
  return
  :error | wmm_werror $addon $scriptline $error | reseterror
...
ON *:INPUT:#: {
  ...
  if (%action) {
    ...
    if (!$2) { .timer -h 1 0 %action ( $++ $wmm_bold($me) $++ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(46)) $+ >) - ( $++ $lang(39) $wmm_bold($1 $wmm_under($wmm_page)) $+ ) | return }
    if (!$wmm_isurl($2)) { .timer -h 1 0 %action ( $++ $wmm_bold($me) $++ ): $lang(54) - $lang(55) - ( $++ $lang(39) $wmm_bold($1 $wmm_under($wmm_page)) $+ ) | return }
...
ON $*:TEXT:$(/^(\Q $+ $replacecs($evalnext($+(%,$mod,_prefix_nick)),\E,\E\\E\Q) $+ \E|\Q $+ $replacecs($evalnext($+(%,$mod,_prefix_chan)),\E,\E\\E\Q) $+ \E).*/Si):#: {
  ...
  if (%action) {
    hadd -mu6 $upper($mod) $+ _FLOOD %cn 1
    if (!$2) { %action ( $++ $wmm_bold($nick) $++ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(46)) $+ >) - ( $++ $lang(39) $wmm_bold($1 $wmm_under($wmm_page)) $+ ) | return }
    if (!$wmm_isurl($2)) { %action ( $++ $wmm_bold($nick) $++ ): $lang(54) - $lang(55) - ( $++ $lang(39) $wmm_bold($1 $wmm_under($wmm_page)) $+ ) | return }
...
alias wus_ver { return $right($gettok($read($script,n,6),3,32),3) }
alias wus_crdate { return $remove($gettok($read($script,n,6),5,32),$chr(40),$chr(41)) }

alias -l mod { return wus }
alias -l command { return url }
...
alias wus_url_search {
  ...
  jsonopen -ud %v https://api.embed.ly/1/extract?key= $+ $wmm_apikey($addon) $+ &format=json&nojsoncallback=1&url= $+ $wmm_urlencode($4-)
  ...
  if ($istok(% [ $+ [ $mod ] $+ ] _show,title,32)) { 
    var %title = $wmm_html2asc($wmm_fixtab($json(%v,title).value))
  ...
  if ($istok(% [ $+ [ $mod ] $+ ] _show,description,32)) { 
    var %desc = $wmm_html2asc($wmm_fixtab($json(%v,description).value))
    if (%desc) && ($len(%desc) >= % [ $+ [ $mod ] $+ ] _desc_chars_max) { var %desc = $left(%desc,$calc(% [ $+ [ $mod ] $+ ] _desc_chars_max -3)) $++ ... }
    if (!%desc) { var %desc = $lang(50) }
  }
  if ($istok(% [ $+ [ $mod ] $+ ] _show,provider,32)) { 
    var %provider = $json(%v,provider_name).value
    if (!%provider) { var %provider = $lang(50) }
  }
  if ($istok(% [ $+ [ $mod ] $+ ] _show,format,32)) {
    var %format = $json(%v,type).value
    if (!%format) { var %format = $lang(50) }
  }

  var %msg = 11,12 $+ $gettok($addon,1,45) $+ -0,14 $+ $gettok($addon,2,45) $+ : $wmm_bel $iif(%title,$wmm_bold($lang(43)) $+ :6 %title $+ ) $iif(%desc,$wmm_sep $wmm_bold($lang(44)) $+ :14 %desc $+ ) $iif(%format,$wmm_sep $wmm_bold($lang(45)) $+ :7 %format $+ ) $iif(%provider,$wmm_sep $wmm_bold($lang(49)) $+ :2 %provider $+ )
...
#wus_menu_status end
#wus_menu_channel off
menu channel { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#wus_menu_channel end
#wus_menu_nicklist off
menu nicklist { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#wus_menu_nicklist end
#wus_menu_query off
menu query { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
