...
dialog -l wps_sets {
  ...
  check "Allow all output links to be shorten", 520, 4 90 154 10, tab 13
...
ON *:INPUT:#: {
  ...
  if (%action) {
    ...
    if (!$2) { .timer -h 1 0 %action ( $++ $wmm_bold($me) $++ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(46)) $+ >) - ( $++ $lang(39) $wmm_bold($1 $wmm_under(IRC)) $+ ) | return }
...
ON $*:TEXT:$(/^(\Q $+ $replacecs($evalnext($+(%,$mod,_prefix_nick)),\E,\E\\E\Q) $+ \E|\Q $+ $replacecs($evalnext($+(%,$mod,_prefix_chan)),\E,\E\\E\Q) $+ \E).*/Si):#: {
  ...
  if (%action) {
    hadd -mu6 $upper($mod) $+ _FLOOD %cn 1
    if (!$2) { %action ( $++ $wmm_bold($nick) $++ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(46)) $+ >) - ( $++ $lang(39) $wmm_bold($1 $wmm_under(IRC)) $+ ) | return }
...
alias wps_ver { return $right($gettok($read($script,n,6),3,32),3) }
alias wps_crdate { return $remove($gettok($read($script,n,6),5,32),$chr(40),$chr(41)) }

alias -l mod { return wps }
alias -l command { return wiki }
...
alias wps_wikipedia_search {
  ...
  var %v = wiki_ $+ $wmm_random

  jsonopen -ud %v https://en.wikipedia.org/w/api.php?action=query&list=search&prop=images $+ $chr(124) $+ info $+ $chr(124) $+ categories&format=json&srsearch= $+ $wmm_urlencode($4-) $+ &srnamespace=0&srprop=snippet&srlimit= $+ % [ $+ [ $mod ] $+ ] _max_results
  if ($jsonerror) { var %m = ( $+ $wmm_bold($1) $+ ): $lang(40) - ( $+ $lang(41) $wmm_bold($jsonerror) $+ ) | %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%m),%m) | return }

  var %tot = $json(%v,query,searchinfo,totalhits).value
  if (!%tot) { var %m = ( $+ $wmm_bold($1) $+ ): $lang(42) | %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%m),%m) | return }
  var %x = 0
  while (%x < % [ $+ [ $mod ] $+ ] _max_results) {

    var %id = $json(%v,query,search,%x,pageid).value
    if (!%id) { goto next }
    var %total = $calc(%total +1)

    if ($istok(% [ $+ [ $mod ] $+ ] _show,title,32)) {
      var %title = $wmm_html2asc($wmm_nohtml($wmm_fixtab($json(%v,query,search,%x,title).value)))
    ...
    if ($istok(% [ $+ [ $mod ] $+ ] _show,description,32)) {
      var %desc = $wmm_html2asc($wmm_nohtml($wmm_fixtab($json(%v,query,search,%x,snippet).value)))
      if (%desc) && ($len(%desc) >= % [ $+ [ $mod ] $+ ] _desc_chars_max) { var %desc = $left(%desc,$calc(% [ $+ [ $mod ] $+ ] _desc_chars_max -3)) $++ ... }
    }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,link,32)) {
      var %url = https://en.wikipedia.org/wiki/ $+ $replace($wmm_html2asc($wmm_nohtml($wmm_fixtab($json(%v,query,search,%x,title).value))),$chr(32),_)
      if (% [ $+ [ $mod ] $+ ] _tiny) { var %url = $wmm_tinycom(%url) }
    }

    var %msg = 0,114W0i14k0i14p0e14d0i14a-0,14 $+ $gettok($addon,2,45) $+ : $wmm_bel $iif(%title,$wmm_bold($lang(43)) $+ :6 %title $+ ) $iif(%desc,$wmm_sep $wmm_bold($lang(44)) $+ :14 %desc $+ ) $iif(%url,$wmm_sep $wmm_bold($lang(45)) $+ :12 $wmm_under(%url) $+ )
    %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%msg),%msg)

    :next
    inc %x
  }
  if (!%total) { var %m = ( $+ $wmm_bold($1) $+ ): $lang(42) | %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%m),%m) | return }
...
#wps_menu_status end
#wps_menu_channel off
menu channel { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#wps_menu_channel end
#wps_menu_nicklist off
menu nicklist { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#wps_menu_nicklist end
#wps_menu_query off
menu query { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
...
#wikipedia-search off
#wikipedia-search end
