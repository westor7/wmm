...
dialog -l wem_sets {
  ...
  check "Show the 'Free' information", 29, 160 18 164 10, tab 13
  check "Show the 'Role' information", 30, 160 28 164 10, tab 13
  check "Show the 'MX' information", 31, 160 38 164 10, tab 13
...
ON *:DIALOG:wem_sets:*:*: {
  if ($devent == init) {
    ...
    if ($istok(% [ $+ [ $mod ] $+ ] _show,free,32)) { did -c $dname 29 }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,role,32)) { did -c $dname 30 }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,mx,32)) { did -c $dname 31 }
  ...
  if ($devent == close) {
    .timer[ $+ $mod $+ _*] off
    if (!% [ $+ [ $mod ] $+ ] _show) { set % $+ $mod $+ _show free mx }
...
ON *:LOAD: {
  ...
  if (% [ $+ [ $mod ] $+ ] _show == $null) { set % $+ $mod $+ _show free role mx }
  hfree -w $mod $+ _*
  .signal -n wmm_close
  return
  :error | wmm_werror $addon $scriptline $error | reseterror
...
ON *:INPUT:#: {
  ...
  if (%action) {
    ...
    if (!$2) || ($numtok($2,64) !== 2) { .timer -h 1 0 %action ( $++ $wmm_bold($me) $++ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(46)) $+ >) - ( $++ $lang(39) $wmm_bold($1 $wmm_under(westor7@gmail.com)) $+ ) | return }
    if ($chr(46) !isin $gettok($2,2,64)) || ($gettok($2,2,64) == $chr(46)) || ($count($2,$chr(64)) !== 1) || ($count($2,$chr(46)) !== 1) || (@. isin $2) || (.@ isin $2) { .timer -h 1 0 %action ( $++ $wmm_bold($me) $++ ): $lang(51) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(46)) $+ >) - ( $++ $lang(39) $wmm_bold($1 $wmm_under(westor7@gmail.com)) $+ ) | return }
...
ON $*:TEXT:$(/^(\Q $+ $replacecs($evalnext($+(%,$mod,_prefix_nick)),\E,\E\\E\Q) $+ \E|\Q $+ $replacecs($evalnext($+(%,$mod,_prefix_chan)),\E,\E\\E\Q) $+ \E).*/Si):#: {
  ...
  if (%action) {
    hadd -mu6 $upper($mod) $+ _FLOOD %cn 1
    if (!$2) || ($numtok($2,64) !== 2) { %action ( $++ $wmm_bold($nick) $++ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(46)) $+ >) - ( $++ $lang(39) $wmm_bold($1 $wmm_under(westor7@gmail.com)) $+ ) | return }
    if ($chr(46) !isin $gettok($2,2,64)) || ($gettok($2,2,64) == $chr(46)) || ($count($2,$chr(64)) !== 1) || ($count($2,$chr(46)) !== 1) || (@. isin $2) || (.@ isin $2) { %action ( $++ $wmm_bold($nick) $++ ): $lang(51) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(46)) $+ >) - ( $++ $lang(39) $wmm_bold($1 $wmm_under(westor7@gmail.com)) $+ ) | return }
...
alias wem_ver { return $right($gettok($read($script,n,6),3,32),3) }
alias wem_crdate { return $remove($gettok($read($script,n,6),5,32),$chr(40),$chr(41)) }

alias -l mod { return wem }
alias -l command { return checkmail }
...
alias wem_email_checker {
  if (!$wmm_internet) || (!$1-) || (!$wmm_apikey($addon)) { return }
  if (msg isin $3) { var %output = $3 $2 }
  elseif (notice isin $3) { var %output = $3 $1 }
  if (!% [ $+ [ $mod ] $+ ] _show) { set % [ $+ [ $mod ] $+ ] _show free mx }
  var %v = email_checker_ $+ $wmm_random

  jsonopen -ud %v https://apilayer.net/api/check?access_key= $+ $wmm_apikey($addon) $+ &email= $+ $4 $+ &smtp=1
  if ($jsonerror) { var %m = ( $+ $wmm_bold($1) $+ ): $lang(40) - ( $+ $lang(41) $wmm_bold($jsonerror) $+ ) | %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%m),%m) | return }

  var %valid = $json(%v,smtp_check).value
  if (!%valid) { var %m = ( $+ $wmm_bold($1) $+ ): $lang(50) | %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%m),%m) | return }
  if ($istok(% [ $+ [ $mod ] $+ ] _show,mx,32)) { var %mx = $json(%v,mx_found).value }
  if ($istok(% [ $+ [ $mod ] $+ ] _show,free,32)) { var %free = $json(%v,free).value }
  if ($istok(% [ $+ [ $mod ] $+ ] _show,role,32)) { var %role = $json(%v,role).value }

  var %msg = 2,11 $+ $gettok($addon,1,45) $+ -0,14 $+ $gettok($addon,2,45) $+ : $wmm_bel $wmm_bold($lang(42)) $+ :3 $lang(54) $+  $iif(%free,$wmm_sep $wmm_bold($lang(43)) $+ :3 $lang(54) $+ ) $iif(%mx,$wmm_sep $wmm_bold($lang(45)) $+ :3 $lang(54) $+ ) $iif(%role,$wmm_sep $wmm_bold($lang(44)) $+ :3 $lang(54) $+ )
...
#wem_menu_status end
#wem_menu_channel off
menu channel { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#wem_menu_channel end
#wem_menu_nicklist off
menu nicklist { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#wem_menu_nicklist end
#wem_menu_query off
menu query { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
