...
dialog -l wis_sets {
  ...
  check "Show the 'Description' information", 31, 160 28 164 10, tab 13
  check "Show the 'Popularity' information", 32, 160 38 164 10, tab 13
  check "Show the 'Votes' information", 33, 160 48 164 10, tab 13
  check "Show the 'Score' information", 34, 160 58 164 10, tab 13
  check "Show the 'Adult' information", 35, 160 68 164 10, tab 13
  check "Show the 'Created on' information", 36, 160 78 164 10, tab 13
  check "Show the 'Link' information", 37, 160 88 164 10, tab 13
  check "Show the 'Language' information", 38, 160 98 164 10, tab 13
  check "Allow all output links to be shorten", 520, 4 80 155 10, tab 13
  check "Display the module output messages", 620, 4 90 150 10, tab 13
  tab "Settings 3", 45
  check "Show empty search informations", 44, 4 50 155 10, tab 45
  check "Show color/bold/underline in informations", 52, 4 60 155 10, tab 45
...
ON *:DIALOG:wis_sets:*:*: {
  if ($devent == init) {
    ...
    did -ra $dname 46 $lang(50) $qt($lang(49)) $lang(59)
    did -ra $dname 48 $lang(50) $qt($lang(51)) $lang(59)
    did -ra $dname 44 $lang(60)
    did -ra $dname 52 $lang(62)
    did -ra $dname 50 $lang(61) $qt($lang(56)) $lang(48)

    did -ra $dname 54 $lang(64) $qt(menubar) $lang(65)
    did -ra $dname 55 $lang(64) $qt(status) $lang(65)
    did -ra $dname 56 $lang(64) $qt(channel) $lang(65)
    did -ra $dname 57 $lang(64) $qt(query) $lang(65)
    did -ra $dname 58 $lang(64) $qt(nicklist) $lang(65)
    did -ra $dname 520 $lang(66)
    ...
    if ($istok(% [ $+ [ $mod ] $+ ] _show,description,32)) { did -c $dname 31 }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,popularity,32)) { did -c $dname 32 }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,votes,32)) { did -c $dname 33 }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,score,32)) { did -c $dname 34 }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,adult,32)) { did -c $dname 35 }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,created_on,32)) { did -c $dname 36 }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,link,32)) { did -c $dname 37 }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,language,32)) { did -c $dname 38 }
    ...
    if (% [ $+ [ $mod ] $+ ] _created_on_format) { did -ra $dname 51 % [ $+ [ $mod ] $+ ] _created_on_format }
  ...
  if ($devent == close) {
    ...
    if (!$did(51)) || (!$wmm_cdate($did(51))) { set % $+ $mod $+ _created_on_format $wmm_d_format }
  ...
  if ($devent == edit) {
    ...
    if ($did == 51) {
      if ($did($did).text) { set % $+ $mod $+ _created_on_format $v1 }
      else { unset % $+ $mod $+ _created_on_format }
    }
  }
  if ($devent == sclick) {
...
ON *:LOAD: {
  ...
  if (% [ $+ [ $mod ] $+ ] _created_on_format == $null) { set % $+ $mod $+ _created_on_format $wmm_d_format }
  if (% [ $+ [ $mod ] $+ ] _show == $null) { set % $+ $mod $+ _show title link popularity votes created_on adult score language }
  hfree -w $mod $+ _*
  .signal -n wmm_close
  return
  :error | wmm_werror $addon $scriptline $error | reseterror
...
ON *:INPUT:#: {
  ...
  if (%action) {
    ...
    if (!$2) { .timer -h 1 0 %action ( $++ $wmm_bold($me) $++ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(46)) $+ >) - ( $++ $lang(39) $wmm_bold($1 $wmm_under(X-men)) $+ ) | return }
...
ON $*:TEXT:$(/^(\Q $+ $replacecs($evalnext($+(%,$mod,_prefix_nick)),\E,\E\\E\Q) $+ \E|\Q $+ $replacecs($evalnext($+(%,$mod,_prefix_chan)),\E,\E\\E\Q) $+ \E).*/Si):#: {
  ...
  if (%action) {
    hadd -mu6 $upper($mod) $+ _FLOOD %cn 1
    if (!$2) { %action ( $++ $wmm_bold($nick) $++ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(46)) $+ >) - ( $++ $lang(39) $wmm_bold($1 $wmm_under(X-men)) $+ ) | return }
...
alias wis_ver { return $right($gettok($read($script,n,6),3,32),3) }
alias wis_crdate { return $remove($gettok($read($script,n,6),5,32),$chr(40),$chr(41)) }

alias -l mod { return wis }
alias -l command { return imdb }
...
alias wis_imdb_search {
  ...
  if (!% [ $+ [ $mod ] $+ ] _created_on_format) || (!$wmm_cdate(% [ $+ [ $mod ] $+ ] _created_on_format)) { set % $+ $mod $+ _created_on_format $wmm_d_format }
  var %v = imdb_ $+ $wmm_random

  jsonopen -ud %v https://api.themoviedb.org/3/search/movie?api_key= $+ $wmm_apikey($addon) $+ &query= $+ $wmm_urlencode($4-) $+ &search_type=ngram&page=1&include_adult=yes
  if ($jsonerror) { var %m = ( $+ $wmm_bold($1) $+ ): $lang(40) - ( $+ $lang(41) $wmm_bold($jsonerror) $+ ) | %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%m),%m) | return }

  var %tot = $json(%v,total_results).value
  if (!%tot) { var %m = ( $+ $wmm_bold($1) $+ ): $lang(42) | %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%m),%m) | return }
  var %x = 0
  while (%x < % [ $+ [ $mod ] $+ ] _max_results) {
    var %id = $json(%v,results,%x,id).value
    if (!%id) { goto next }
    var %total = $calc(%total +1)

    if ($istok(% [ $+ [ $mod ] $+ ] _show,title,32)) { 
      var %title = $wmm_html2asc($wmm_fixtab($json(%v,results,%x,original_title).value))
      if (!%title) { var %title = $wmm_html2asc($wmm_fixtab($json(%v,results,%x,original_name).value)) }
      if (%title) && ($len(%title) >= % [ $+ [ $mod ] $+ ] _title_chars_max) { var %title = $left(%title,$calc(% [ $+ [ $mod ] $+ ] _title_chars_max -3)) $++ ... }
      if (!%title) { var %title = $lang(63) }
    }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,description,32)) { 
      var %desc = $wmm_html2asc($wmm_fixtab($json(%v,results,%x,overview).value))
      if (%desc) && ($len(%desc) >= % [ $+ [ $mod ] $+ ] _desc_chars_max) { var %desc = $left(%desc,$calc(% [ $+ [ $mod ] $+ ] _desc_chars_max -3)) $++ ... }
      if (!%desc) { var %desc = $lang(63) }
    }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,popularity,32)) { 
      var %pop = $int($json(%v,results,%x,popularity).value)
      if (%pop) { var %pop = %pop $+ % }
    }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,votes,32)) { var %votes = $json(%v,results,%x,vote_count).value }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,score,32)) { 
      var %score = $json(%v,results,%x,vote_average).value
      if (%score) { var %score = %score  $+ /10 }
    }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,adult,32)) { 
      var %ad = $json(%v,results,%x,adult).value
      if (%ad) { var %adult = $lang(44) }
      else { var %adult = $lang(45) }
    }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,created_on,32)) { 
      var %date = $json(%v,results,%x,release_date).value
      var %ctime = $ctime(%date)
      if (%ctime) { var %crdate = $date(%ctime,% [ $+ [ $mod ] $+ ] _created_on_format) }
      else { var %crdate = %date }
    }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,language,32)) { 
      var %lang = $json(%v,results,%x,original_language).value
      if (!%lang) { var %lang = $lang(63) }
    }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,link,32)) { 
      var %url = https://www.themoviedb.org/movie/ $+ %id
      if (% [ $+ [ $mod ] $+ ] _tiny) && (%url) { var %url = $wmm_tinycom(%url) }
      if (!%url) { var %url = $lang(63) }
    }

    if (% [ $+ [ $mod ] $+ ] _show_empty) { 
      var %msg = 1,8 $+ $gettok($addon,1,45) $+ -0,14 $+ $gettok($addon,2,45) $+ : $wmm_bel $iif(%title !== $null,$wmm_bold($lang(49)) $+ :6 %title $+ ) $iif(%desc !== $null,$wmm_sep $wmm_bold($lang(51)) $+ :14 %desc $+ ) $iif(%lang !== $null,$wmm_sep $wmm_bold($lang(58)) $+ :2 %lang $+ ) $iif(%adult !== $null,$wmm_sep $wmm_bold($lang(53)) $+ :4 %adult $+ ) $iif(%pop !== $null,$wmm_sep $wmm_bold($lang(54)) $+ :13 %pop $+ ) $iif(%votes !== $null,$wmm_sep $wmm_bold($iif(%votes == 1,$lang(43),$lang(55))) $+ :10 $gettok($bytes(%votes,bd),1,46) $+ ) $iif(%score !== $null,$wmm_sep $wmm_bold($lang(52)) $+ :3 %score $+ ) $iif(%crdate !== $null,$wmm_sep $wmm_bold($lang(56)) $+ :7 %crdate $+ ) $iif(%url !== $null,$wmm_sep $wmm_bold($lang(57)) $+ :12 $wmm_under(%url) $+ )
      %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%msg),%msg)
    }
    elseif (!% [ $+ [ $mod ] $+ ] _show_empty) { 
      var %msg = 1,8 $+ $gettok($addon,1,45) $+ -0,14 $+ $gettok($addon,2,45) $+ : $wmm_bel $iif(%title,$wmm_bold($lang(49)) $+ :6 %title $+ ) $iif(%desc,$wmm_sep $wmm_bold($lang(51)) $+ :14 %desc $+ ) $iif(%lang,$wmm_sep $wmm_bold($lang(58)) $+ :2 %lang $+ ) $iif(%adult,$wmm_sep $wmm_bold($lang(53)) $+ :4 %adult $+ ) $iif(%pop,$wmm_sep $wmm_bold($lang(54)) $+ :13 %pop $+ ) $iif(%votes,$wmm_sep $wmm_bold($iif(%votes == 1,$lang(43),$lang(55))) $+ :10 $gettok($bytes(%votes,bd),1,46) $+ ) $iif(%score,$wmm_sep $wmm_bold($lang(52)) $+ :3 %score $+ ) $iif(%crdate,$wmm_sep $wmm_bold($lang(56)) $+ :7 %crdate $+ ) $iif(%url,$wmm_sep $wmm_bold($lang(57)) $+ :12 $wmm_under(%url) $+ )
      %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%msg),%msg)
    }
...
#wis_menu_status end
#wis_menu_channel off
menu channel { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#wis_menu_channel end
#wis_menu_nicklist off
menu nicklist { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#wis_menu_nicklist end
#wis_menu_query off
menu query { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
