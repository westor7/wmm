...
dialog -l wms_sets {
  ...
  edit "", 51, 160 29 90 9, tab 45 autohs center
...
ON *:DIALOG:wms_sets:*:*: {
  if ($devent == init) {
    ...
    if ($istok(% [ $+ [ $mod ] $+ ] _show,listeners,32)) { did -c $dname 34 }
  ...
  if ($devent == close) {
    ...
    if (!$did(51)) { set % $+ $mod $+ _uploaded_on_format $wmm_d_format }
...
ON *:LOAD: { 
  ...
  if (% [ $+ [ $mod ] $+ ] _show == $null) { set % $+ $mod $+ _show title length views listeners comments link }
  hfree -w $mod $+ _*
  .signal -n wmm_close
  return
  :error | wmm_werror $addon $scriptline $error | reseterror
...
alias wms_ver { return $right($gettok($read($script,n,6),3,32),3) }
alias wms_crdate { return $remove($gettok($read($script,n,6),5,32),$chr(40),$chr(41)) }

alias -l mod { return wms }
alias -l command { return mixcloud }
...
alias wms_mixcloud_search {
  ...
  while (%x < % [ $+ [ $mod ] $+ ] _max_results) {
    ...
    if ($istok(% [ $+ [ $mod ] $+ ] _show,length,32)) { 
      var %duration = $duration($json(%v,data,%x,audio_length).value,3)
    ...
    if ($istok(% [ $+ [ $mod ] $+ ] _show,uploaded_on,32)) {
      var %uploaded = $date($wmm_convertdate($json(%v,data,%x,created_time).value),% [ $+ [ $mod ] $+ ] _uploaded_on_format)
      if (!%uploaded) { var %uploaded = $lang(67) }
    }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,link,32)) {
      var %url = $json(%v,data,%x,url).value
      if (% [ $+ [ $mod ] $+ ] _tiny) { var %url = $wmm_tinycom(%url) }
      if (!%url) { var %url = $lang(67) }
    }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,favorites,32)) { var %favorite = $json(%v,data,%x,favorite_count).value }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,views,32)) { var %views = $json(%v,data,%x,play_count).value }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,comments,32)) { var %comments = $json(%v,data,%x,comment_count).value }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,reposts,32)) { var %reposts = $json(%v,data,%x,repost_count).value }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,listeners,32)) { var %listeners = $json(%v,data,%x,listener_count).value }

    if (% [ $+ [ $mod ] $+ ] _show_empty) {
      var %msg = 0,1 $+ $gettok($addon,1,45) $+ -0,14 $+ $gettok($addon,2,45) $+ : $wmm_bel $iif(%title !== $null,$wmm_bold($lang(49)) $+ :6 %title $+ ) $iif(%duration !== $null,$wmm_sep $wmm_bold($lang(50)) $+ :10 %duration $+ ) $iif(%views !== $null,$wmm_sep $wmm_bold($iif(%views == 1,$lang(43),$lang(51))) $+ :13 $gettok($bytes(%views,bd),1,46) $+ ) $iif(%listeners !== $null,$wmm_sep $wmm_bold($iif(%listeners == 1,$lang(44),$lang(52))) $+ :3 $gettok($bytes(%listeners,bd),1,46) $+ ) $iif(%favorite !== $null,$wmm_sep $wmm_bold($iif(%favorite == 1,$lang(59),$lang(54))) $+ :5 $gettok($bytes(%favorite,bd),1,46) $+ ) $iif(%reposts !== $null,$wmm_sep $wmm_bold($iif(%reposts == 1,$lang(45),$lang(53))) $+ :9 $gettok($bytes(%reposts,bd),1,46) $+ ) $iif(%comments !== $null,$wmm_sep $wmm_bold($iif(%comments == 1,$lang(60),$lang(55))) $+ :10 $gettok($bytes(%comments,bd),1,46) $+ ) $iif(%uploader !== $null,$wmm_sep $wmm_bold($lang(57)) $+ :2 %uploader $+ ) $iif(%uploaded !== $null,$wmm_sep $wmm_bold($lang(56)) $+ :7 %uploaded $+ ) $iif(%url !== $null,$wmm_sep $wmm_bold($lang(58)) $+ :12 $wmm_under(%url) $+ )
      %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%msg),%msg)
    }
    elseif (!% [ $+ [ $mod ] $+ ] _show_empty) {
      var %msg = 0,1 $+ $gettok($addon,1,45) $+ -0,14 $+ $gettok($addon,2,45) $+ : $wmm_bel $iif(%title,$wmm_bold($lang(49)) $+ :6 %title $+ ) $iif(%duration,$wmm_sep $wmm_bold($lang(50)) $+ :10 %duration $+ ) $iif(%views,$wmm_sep $wmm_bold($iif(%views == 1,$lang(43),$lang(51))) $+ :13 $gettok($bytes(%views,bd),1,46) $+ ) $iif(%listeners,$wmm_sep $wmm_bold($iif(%listeners == 1,$lang(44),$lang(52))) $+ :3 $gettok($bytes(%listeners,bd),1,46) $+ ) $iif(%favorite,$wmm_sep $wmm_bold($iif(%favorite == 1,$lang(59),$lang(54))) $+ :5 $gettok($bytes(%favorite,bd),1,46) $+ ) $iif(%reposts,$wmm_sep $wmm_bold($iif(%reposts == 1,$lang(45),$lang(53))) $+ :9 $gettok($bytes(%reposts,bd),1,46) $+ ) $iif(%comments,$wmm_sep $wmm_bold($iif(%comments == 1,$lang(60),$lang(55))) $+ :10 $gettok($bytes(%comments,bd),1,46) $+ ) $iif(%uploader,$wmm_sep $wmm_bold($lang(57)) $+ :2 %uploader $+ ) $iif(%uploaded,$wmm_sep $wmm_bold($lang(56)) $+ :7 %uploaded $+ ) $iif(%url,$wmm_sep $wmm_bold($lang(58)) $+ :12 $wmm_under(%url) $+ )
      %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%msg),%msg)
    }
...
#wms_menu_status end
#wms_menu_channel off
menu channel { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#wms_menu_channel end
#wms_menu_nicklist off
menu nicklist { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#wms_menu_nicklist end
#wms_menu_query off
menu query { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
