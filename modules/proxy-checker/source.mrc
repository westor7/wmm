...
dialog -l wxc_sets {
  ...
  check "Show the 'ASN' information", 29, 160 18 164 10, tab 13
  check "Show the 'Provider' information", 30, 160 28 164 10, tab 13
  check "Show the 'Country Name' information", 31, 160 38 164 10, tab 13
  check "Show the 'Country Code' information", 32, 160 48 164 10, tab 13
  check "Show the 'Proxy' information", 33, 160 58 164 10, tab 13
  check "Show the 'Proxy Type' information", 34, 160 68 164 10, tab 13
  check "Show the 'IP Type' information", 35, 160 78 164 10, tab 13
...
ON *:DIALOG:wxc_sets:*:*: {
  if ($devent == init) {
    ...
    did -ra $dname 33 $lang(51) $qt($lang(56)) $lang(52)
    did -ra $dname 34 $lang(51) $qt($lang(57)) $lang(52)
    did -ra $dname 35 $lang(51) $qt($lang(59)) $lang(52)
    ...
    if ($istok(% [ $+ [ $mod ] $+ ] _show,asn,32)) { did -c $dname 29 }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,provider,32)) { did -c $dname 30 }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,country_name,32)) { did -c $dname 31 }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,country_code,32)) { did -c $dname 32 }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,proxy,32)) { did -c $dname 33 }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,proxy_type,32)) { did -c $dname 34 }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,ip_type,32)) { did -c $dname 35 }
  ...
  if ($devent == close) {
    .timer[ $+ $mod $+ _*] off
    if (!% [ $+ [ $mod ] $+ ] _show) { set % $+ $mod $+ _show proxy proxy_type }
...
ON *:LOAD: { 
  ...
  if (% [ $+ [ $mod ] $+ ] _show == $null) { set % $+ $mod $+ _show asn provider country_name country_code proxy proxy_type ip_type }
  hfree -w $mod $+ _*
  .signal -n wmm_close
  return
  :error | wmm_werror $addon $scriptline $error | reseterror
...
ON *:INPUT:#: {
  ...
  if (%action) {
    ...
    if (!$2) { .timer -h 1 0 %action ( $++ $wmm_bold($me) $++ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(40)) $+ >) - ( $+ $lang(39) $wmm_bold($1 $wmm_under($serverip)) $+ ) | return }
    if (!$wmm_check_ipv4($2)) && (!$wmm_check_ipv6($2)) { .timer -h 1 0 %action ( $++ $wmm_bold($me) $++ ): $lang(54) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(40)) $+ >) - ( $+ $lang(39) $wmm_bold($1 $wmm_under($serverip)) $+ ) | return }
...
ON $*:TEXT:$(/^(\Q $+ $replacecs($evalnext($+(%,$mod,_prefix_nick)),\E,\E\\E\Q) $+ \E|\Q $+ $replacecs($evalnext($+(%,$mod,_prefix_chan)),\E,\E\\E\Q) $+ \E).*/Si):#: {
  ...
  if (%action) {
    hadd -mu6 $upper($mod) $+ _FLOOD %cn 1
    if (!$2) { %action ( $++ $wmm_bold($nick) $++ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(40)) $+ >) - ( $+ $lang(39) $wmm_bold($1 $wmm_under($serverip)) $+ ) | return }
    if (!$wmm_check_ipv4($2)) && (!$wmm_check_ipv6($2)) { %action ( $++ $wmm_bold($nick) $++ ): $lang(54) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(40)) $+ >) - ( $+ $lang(39) $wmm_bold($1 $wmm_under($serverip)) $+ ) | return }
...
alias wxc_ver { return $right($gettok($read($script,n,6),3,32),3) }
alias wxc_crdate { return $remove($gettok($read($script,n,6),5,32),$chr(40),$chr(41)) }

alias -l mod { return wxc }
alias -l command { return proxy }
...
alias wxc_proxy_checker {
  if (!$wmm_internet) || (!$1-) || (!$wmm_apikey($addon)) { return }
  if (msg isin $3) { var %output = $3 $2 }
  elseif (notice isin $3) { var %output = $3 $1 }
  if (!% [ $+ [ $mod ] $+ ] _show) { set % $+ $mod $+ _show proxy proxy_type }
  var %v = proxy_checker_ $+ $wmm_random

  jsonopen -ud %v https://proxycheck.io/v2/ $+ $4 $+ ?key= $+ $wmm_apikey($addon) $+ &vpn=1&asn=1&time=1&inf=1&days=7
  ...
  var %error_msg = $json(%v,message).value
  if (%status == error) && (%error_msg) { var %m = ( $+ $wmm_bold($1) $+ ): $lang(46) - ( $+ $lang(45) $wmm_bold(%error_msg) $+ ) | %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%m),%m) | return }

  if ($istok(% [ $+ [ $mod ] $+ ] _show,asn,32)) { var %asn = $json(%v,$4,asn).value }
  if ($istok(% [ $+ [ $mod ] $+ ] _show,provider,32)) { var %provider = $json(%v,$4,provider).value }
  if ($istok(% [ $+ [ $mod ] $+ ] _show,country_name,32)) { var %country_name = $json(%v,$4,country).value }
  if ($istok(% [ $+ [ $mod ] $+ ] _show,country_code,32)) { var %country_code = $json(%v,$4,isocode).value }
  if ($istok(% [ $+ [ $mod ] $+ ] _show,ip_type,32)) { var %ip_type = $iptype($4) }
  if ($istok(% [ $+ [ $mod ] $+ ] _show,proxy,32)) { 
    var %proxy = $json(%v,$4,proxy).value
    if (%proxy == yes) { var %proxy = $lang(60) }
    if (%proxy == no) { var %proxy = $lang(61) }
  }
  if ($istok(% [ $+ [ $mod ] $+ ] _show,proxy_type,32)) { 
    var %proxy_type = $json(%v,$4,type).value
    if (!%proxy_type) { var %proxy_type = $lang(58) }
  }

  var %msg = 7,8 $+ $gettok($addon,1,45) $+ -0,14 $+ $gettok($addon,2,45) $+ : $wmm_bel $iif(%country_name,$wmm_bold($lang(49)) $+ :6 %country_name $+ ) $iif(%country_code,$wmm_sep $wmm_bold($lang(50)) $+ :5 %country_code $+ ) $iif(%provider,$wmm_sep $wmm_bold($lang(48)) $+ :7 %provider $+ ) $iif(%asn,$wmm_sep $wmm_bold($lang(47)) $+ :2 %asn $+ ) $iif(%proxy,$wmm_sep $wmm_bold($lang(56)) $+ : $+ $iif(%proxy == yes,3,4) %proxy $+ ) $iif(%proxy_type,$wmm_sep $wmm_bold($lang(57)) $+ :13 %proxy_type $+ ) $iif(%ip_type,$wmm_sep $wmm_bold($lang(59)) $+ :10 %ip_type $+ )
  %output $iif(%wxc_strip,$strip(%msg),%msg)
...
#wxc_menu_status end
#wxc_menu_channel off
menu channel { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#wxc_menu_channel end
#wxc_menu_nicklist off
menu nicklist { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#wxc_menu_nicklist end
#wxc_menu_query off
menu query { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
