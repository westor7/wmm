...
dialog -l wbp_sets {
  ...
  check "Display the wrong word next to the correct word", 30, 4 75 180 10, tab 13
  check "Show color/bold/underline in informations", 52, 4 85 155 10, tab 13
  check "Display the module output messages", 620, 4 95 155 10, tab 13
  tab "Settings 3", 53
...
ON *:DIALOG:wbp_sets:*:*: {
  if ($devent == init) {
    ...
    did -ra $dname 30 $lang(44)

    did -ra $dname 52 $lang(45)
    did -ra $dname 620 $lang(51)

    did -ra $dname 53 $lang(4) 3
    did -ra $dname 54 $lang(49) $qt(menubar) $lang(50)
    did -ra $dname 55 $lang(49) $qt(status) $lang(50)
    did -ra $dname 56 $lang(49) $qt(channel) $lang(50)
    did -ra $dname 57 $lang(49) $qt(query) $lang(50)
    did -ra $dname 58 $lang(49) $qt(nicklist) $lang(50)

    if ($istok(% [ $+ [ $mod ] $+ ] _show,wrong_words,32)) { did -c $dname 30 }
...
ON *:LOAD: { 
  ...
  if (% [ $+ [ $mod ] $+ ] _show == $null) { set % $+ $mod $+ _show wrong_words }
  hfree -w $mod $+ _*
  .signal -n wmm_close
  return
  :error | wmm_werror $addon $scriptline $error | reseterror
...
ON *:INPUT:#: {
  ...
  if (%action) {
    ...
    if (!$2) { .timer -h 1 0 %action ( $++ $wmm_bold($me) $++ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(46)) $+ >) - ( $++ $lang(39) $wmm_bold($1 $wmm_under(goverment)) $+ ) | return }
...
ON $*:TEXT:$(/^(\Q $+ $replacecs($evalnext($+(%,$mod,_prefix_nick)),\E,\E\\E\Q) $+ \E|\Q $+ $replacecs($evalnext($+(%,$mod,_prefix_chan)),\E,\E\\E\Q) $+ \E).*/Si):#: {
  ...
  if (%action) {
    hadd -mu6 $upper($mod) $+ _FLOOD %cn 1
    if (!$2) { %action ( $++ $wmm_bold($nick) $++ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(46)) $+ >) - ( $++ $lang(39) $wmm_bold($1 $wmm_under(goverment)) $+ ) | return }
...
alias wbp_ver { return $right($gettok($read($script,n,6),3,32),3) }
alias wbp_crdate { return $remove($gettok($read($script,n,6),5,32),$chr(40),$chr(41)) }

alias -l mod { return wbp }
alias -l command { return spell }
...
alias wbp_bing_spell {
  if (!$wmm_internet) || (!$1-) || (!$wmm_apikey($addon)) { return }
  if (msg isin $3) { var %output = $3 $2 }
  elseif (notice isin $3) { var %output = $3 $1 }
  var %v = spell_ $+ $wmm_random

  jsonopen -wud %v https://montanaflynn-spellcheck.p.mashape.com/check/?text= $+ $wmm_urlencode($4-)
  if ($jsonerror) { var %m = ( $+ $wmm_bold($1) $+ ): $lang(40) - ( $+ $lang(41) $wmm_bold($jsonerror) $+ ) | %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%m),%m) | return }
  jsonhttpheader %v X-Mashape-Key $wmm_apikey($addon)
  jsonhttpheader %v Accept application/json
  jsonhttpfetch %v

  var %cor_text = $4-
  var %tot = $json(%v,corrections).length
  if (!%tot) { var %m = ( $+ $wmm_bold($1) $+ ): $lang(42) | %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%m),%m) | return }

  var %x = 0
  while (%x < %tot) {
    var %key = $json(%v, corrections, ~ %x).fuzzy
    var %old = $jsonpath(%key, $jsonpath(%key, 0))
    var %new = $json(%v, corrections, %old, 0).value

    var %old_c = 4 $+ $wmm_italic(%old) $+ 6
    var %new_c = $iif($istok(%wbp_show,wrong_words,32),$chr(40) $+ %old_c $+ $chr(41) 3 $+ %new $+ 6,3 $+ %new $+ 6)
    var %cor_text = $replace(%cor_text,%old,%new_c)

    inc %x
  }

  var %msg = 0,10 $+ $gettok($addon,1,45) $+ -0,14 $+ $gettok($addon,2,45) $+ : $wmm_bel $iif(%cor_text,$wmm_bold($lang(43)) $+ :6 %cor_text $+ )
...
#wbp_menu_status end
#wbp_menu_channel off
menu channel { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#wbp_menu_channel end
#wbp_menu_nicklist off
menu nicklist { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#wbp_menu_nicklist end
#wbp_menu_query off
menu query { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
