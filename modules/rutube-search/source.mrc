...
dialog -l wrt_sets {
  ...
  check "Show the 'Definition' information", 34, 160 48 164 10, tab 13
  check "Show the 'Uploaded on' information", 36, 160 58 164 10, tab 13
  check "Show the 'Uploaded by' information", 37, 160 68 164 10, tab 13
  check "Show the 'Link' information", 38, 160 78 164 10, tab 13
  check "Show the 'Length' information", 39, 160 88 164 10, tab 13
...
ON *:DIALOG:wrt_sets:*:*: {
  if ($devent == init) {
    ...
    did -ra $dname 50 $lang(60) $qt($lang(56)) $lang(48)
    ...
    if ($istok(% [ $+ [ $mod ] $+ ] _show,definition,32)) { did -c $dname 34 }
...
ON *:LOAD: {
  ...
  if (% [ $+ [ $mod ] $+ ] _show == $null) { set % $+ $mod $+ _show title length views link uploaded_on uploaded_by }
  hfree -w $mod $+ _*
  .signal -n wmm_close
  return
  :error | wmm_werror $addon $scriptline $error | reseterror
...
alias wrt_ver { return $right($gettok($read($script,n,6),3,32),3) }
alias wrt_crdate { return $remove($gettok($read($script,n,6),5,32),$chr(40),$chr(41)) }

alias -l mod { return wrt }
alias -l command { return rutube }
...
alias wrt_rutube_search {
  ...
  var %v = rutube_ $+ $wmm_random

  jsonopen -ud %v https://rutube.ru/api/search/video/?query= $+ $wmm_urlencode($4-) $+ &format=json&page=1
  if ($jsonerror) { var %m = ( $+ $wmm_bold($1) $+ ): $lang(40) - ( $+ $lang(41) $wmm_bold($jsonerror) $+ ) | %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%m),%m) | return }

  var %tot = $json(%v,count).value
  ...
  while (%x < % [ $+ [ $mod ] $+ ] _max_results) {
    ...
    if ($istok(% [ $+ [ $mod ] $+ ] _show,title,32)) { 
      var %title = $wmm_html2asc($wmm_fixtab($json(%v,results,%x,title).value))
    ...
    if ($istok(% [ $+ [ $mod ] $+ ] _show,description,32)) { 
      var %desc = $wmm_html2asc($wmm_fixtab($json(%v,results,%x,description).value))
      if (%desc) && ($len(%desc) >= % [ $+ [ $mod ] $+ ] _desc_chars_max) { var %desc = $left(%desc,$calc(% [ $+ [ $mod ] $+ ] _desc_chars_max -3)) $++ ... }
    }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,length,32)) { var %duration = $duration($json(%v,results,%x,duration).value,3) }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,definition,32)) { var %hd = $json(%v,results,%x,has_high_quality).value }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,views,32)) { var %views = $json(%v,results,%x,views).value }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,uploaded_by,32)) { var %uploader = $json(%v,results,%x,author,name).value }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,uploaded_on,32)) { var %uploaded = $date($wmm_convertdate($json(%v,results,%x,created_ts).value),% [ $+ [ $mod ] $+ ] _uploaded_on_format) }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,link,32)) { 
      var %url = https://rutube.ru/video/ $+ %id
      if (% [ $+ [ $mod ] $+ ] _tiny) { var %url = $wmm_tinycom(%url) }
    }

    var %msg = 0,3RU0,1Tube-0,14 $+ $gettok($addon,2,45) $+ : $wmm_bel $iif(%title,$wmm_bold($lang(49)) $+ :6 %title $+ ) $iif(%desc,$wmm_sep $wmm_bold($lang(44)) $+ :14 %desc $+ ) $iif(%duration,$wmm_sep $wmm_bold($lang(50)) $+ :10 %duration $+ ) $iif(%hd,$wmm_sep $wmm_bold($lang(54)) $+ :13 HD $+ ) $iif(%views,$wmm_sep $wmm_bold($iif(%views == 1,$lang(43),$lang(51))) $+ :3 $gettok($bytes(%views,bd),1,46) $+ ) $iif(%uploader,$wmm_sep $wmm_bold($lang(57)) $+ :2 %uploader $+ ) $iif(%uploaded,$wmm_sep $wmm_bold($lang(56)) $+ :7 %uploaded $+ ) $iif(%url,$wmm_sep $wmm_bold($lang(58)) $+ :12 $wmm_under(%url) $+ )
...
#wrt_menu_status end
#wrt_menu_channel off
menu channel { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#wrt_menu_channel end
#wrt_menu_nicklist off
menu nicklist { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#wrt_menu_nicklist end
#wrt_menu_query off
menu query { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
