...
dialog -l wdm_sets {
  ...
  check "Show the 'Type' information", 33, 160 58 164 10, tab 13
  check "Show the 'Language' information", 34, 160 68 164 10, tab 13
...
ON *:DIALOG:wdm_sets:*:*: {
  if ($devent == init) {
    ...
    if ($istok(% [ $+ [ $mod ] $+ ] _show,type,32)) { did -c $dname 33 }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,language,32)) { did -c $dname 34 }
...
ON *:LOAD: {
  ...
  if (% [ $+ [ $mod ] $+ ] _show == $null) { set % $+ $mod $+ _show title length views likes comments link }
  hfree -w $mod $+ _*
  .signal -n wmm_close
  return
  :error | wmm_werror $addon $scriptline $error | reseterror
...
alias wdm_ver { return $right($gettok($read($script,n,6),3,32),3) }
alias wdm_crdate { return $remove($gettok($read($script,n,6),5,32),$chr(40),$chr(41)) }

alias -l mod { return wdm }
alias -l command { return dailymotion }
...
alias wdm_dailymotion_search {
  ...
  var %v = dailymotion_ $+ $wmm_random

  jsonopen -ud %v https://api.dailymotion.com/videos?search=  $+ $wmm_urlencode($4-) $+ &limit= $+ % [ $+ [ $mod ] $+ ] _max_results
  ...
  while (%i <= $numtok(%ids,32)) {
    var %vv = dailymotion_ids_ $+ $wmm_random
    var %id = $gettok(%ids,%i,32)
    if (!%id) { goto next }
    var %total = $calc(%total +1)

    jsonopen -ud %vv https://api.dailymotion.com/video/ $+ %id $+ ?fields=channel,comments_total,created_time,description,duration,language,likes_total,owner.username,tiny_url,url,title,views_total
    if ($jsonerror) { var %m = ( $+ $wmm_bold($1) $+ ): $lang(40) - ( $+ $lang(41) $wmm_bold($jsonerror) $+ ) | %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%m),%m) | return }

    if ($istok(% [ $+ [ $mod ] $+ ] _show,title,32)) { 
      var %title = $wmm_html2asc($wmm_fixtab($json(%vv,title).value))
    ...
    if ($istok(% [ $+ [ $mod ] $+ ] _show,description,32)) { 
      var %desc = $wmm_html2asc($wmm_nohtml($wmm_fixtab($left($json(%vv,description).value,4000))))
    ...
    if ($istok(% [ $+ [ $mod ] $+ ] _show,uploaded_on,32)) { 
      var %uploaded = $json(%vv,created_time).value 
      if (%uploaded) { var %uploaded = $date(%uploaded,% [ $+ [ $mod ] $+ ] _uploaded_on_format) }
      if (!%uploaded) { var %uploaded = $lang(59) }
    }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,uploaded_by,32)) { 
      var %uploader = $json(%vv,owner.username).value
      if (!%uploader) { var %uploader = $lang(59) }
    }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,language,32)) { 
      var %language = $json(%vv,language).value
      if (!%language) { var %language = $lang(59) }
    }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,length,32)) { 
      var %duration = $json(%vv,duration).value
      if (%duration) { var %duration = $duration(%duration,3) }
      if (!%duration) { var %duration = 00:00:00 }
    }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,type,32)) { 
      var %type = $json(%vv,channel).value
      if (!%type) { var %type = $lang(59) }
    }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,views,32)) { var %views = $json(%vv,views_total).value }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,likes,32)) { var %likes = $json(%vv,likes_total).value }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,comments,32)) { var %comments = $json(%vv,comments_total).value }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,link,32)) { 
      if (% [ $+ [ $mod ] $+ ] _tiny) { var %url = $json(%vv,tiny_url).value }
      elseif (!% [ $+ [ $mod ] $+ ] _tiny) { var %url = $json(%vv,url).value }
      if (!%url) { var %url = $lang(59) }
    }

    if (% [ $+ [ $mod ] $+ ] _show_empty) {
      var %msg = 0,12Daily0,2Motion-0,14 $+ $gettok($addon,2,45) $+ : $wmm_bel $iif(%title !== $null,$wmm_bold($lang(49)) $+ :6 %title $+ ) $iif(%desc !== $null,$wmm_sep $wmm_bold($lang(65)) $+ :14 %desc $+ ) $iif(%duration !== $null,$wmm_sep $wmm_bold($lang(50)) $+ :10 %duration $+ ) $iif(%language !== $null,$wmm_sep $wmm_bold($lang(54)) $+ :13 %language $+ ) $iif(%views !== $null,$wmm_sep $wmm_bold($iif(%views == 1,$lang(43),$lang(51))) $+ :3 $gettok($bytes(%views,bd),1,46) $+ ) $iif(%likes !== $null,$wmm_sep $wmm_bold($iif(%likes == 1,$lang(44),$lang(52))) $+ :9 $gettok($bytes(%likes,bd),1,46) $+ ) $iif(%type !== $null,$wmm_sep $wmm_bold($lang(45)) $+ :4 %type $+ ) $iif(%comments !== $null,$wmm_sep $wmm_bold($iif(%comments == 1,$lang(53),$lang(55))) $+ :12 $gettok($bytes(%comments,bd),1,46) $+ ) $iif(%uploader !== $null,$wmm_sep $wmm_bold($lang(57)) $+ :2 %uploader $+ ) $iif(%uploaded !== $null,$wmm_sep $wmm_bold($lang(56)) $+ :7 %uploaded $+ ) $iif(%url !== $null,$wmm_sep $wmm_bold($lang(58)) $+ :12 $wmm_under(%url) $+ )
      %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%msg),%msg)
    }
    elseif (!% [ $+ [ $mod ] $+ ] _show_empty) { 
      var %msg = 0,12Daily0,2Motion-0,14 $+ $gettok($addon,2,45) $+ : $wmm_bel $iif(%title,$wmm_bold($lang(49)) $+ :6 %title $+ ) $iif(%desc,$wmm_sep $wmm_bold($lang(65)) $+ :14 %desc $+ ) $iif(%duration,$wmm_sep $wmm_bold($lang(50)) $+ :10 %duration $+ ) $iif(%language,$wmm_sep $wmm_bold($lang(54)) $+ :13 %language $+ ) $iif(%views,$wmm_sep $wmm_bold($iif(%views == 1,$lang(43),$lang(51))) $+ :3 $gettok($bytes(%views,bd),1,46) $+ ) $iif(%likes,$wmm_sep $wmm_bold($iif(%likes == 1,$lang(44),$lang(52))) $+ :9 $gettok($bytes(%likes,bd),1,46) $+ ) $iif(%type,$wmm_sep $wmm_bold($lang(45)) $+ :4 %type $+ ) $iif(%comments,$wmm_sep $wmm_bold($iif(%comments == 1,$lang(53),$lang(55))) $+ :12 $gettok($bytes(%comments,bd),1,46) $+ ) $iif(%uploader,$wmm_sep $wmm_bold($lang(57)) $+ :2 %uploader $+ ) $iif(%uploaded,$wmm_sep $wmm_bold($lang(56)) $+ :7 %uploaded $+ ) $iif(%url,$wmm_sep $wmm_bold($lang(58)) $+ :12 $wmm_under(%url) $+ )
      %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%msg),%msg)
    }
...
#wdm_menu_status end
#wdm_menu_channel off
menu channel { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#wdm_menu_channel end
#wdm_menu_nicklist off
menu nicklist { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#wdm_menu_nicklist end
#wdm_menu_query off
menu query { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
...
#dailymotion-search off
#dailymotion-search end
