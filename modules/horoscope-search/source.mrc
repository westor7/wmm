...
dialog -l whs_sets {
  ...
  check "Display the module output messages", 620, 4 100 155 10, tab 13
  tab "Settings 3", 53
...
ON *:DIALOG:whs_sets:*:*: {
  if ($devent == init) {
    ...
    if ($istok(% [ $+ [ $mod ] $+ ] _show,description,32)) { did -c $dname 29 }
  ...
  if ($devent == close) {
    .timer[ $+ $mod $+ _*] off
    if (!% [ $+ [ $mod ] $+ ] _show) { set % $+ $mod $+ _show description link }
...
ON *:LOAD: {  
  ...
  if (% [ $+ [ $mod ] $+ ] _show == $null) { set % $+ $mod $+ _show description link }
  hfree -w $mod $+ _*
  .signal -n wmm_close
  return
  :error | wmm_werror $addon $scriptline $error | reseterror
...
ON *:INPUT:#: {
  ...
  if (%action) {
    ...
    if (!$2) { .timer -h 1 0 %action ( $++ $wmm_bold($me) $++ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(46)) $+ >) - ( $++ $lang(39) $wmm_bold($1 $wmm_under($gettok($horos,$rand(1,12),32))) $+ ) | return }
    if (!$istok($horos,$2,32)) { .timer -h 1 0 %action ( $++ $wmm_bold($me) $++ ): $lang(43) $wmm_bold($horos) - ( $++ $lang(39) $wmm_bold($1 $wmm_under($gettok($horos,$rand(1,12),32))) $+ ) | return }
...
ON $*:TEXT:$(/^(\Q $+ $replacecs($evalnext($+(%,$mod,_prefix_nick)),\E,\E\\E\Q) $+ \E|\Q $+ $replacecs($evalnext($+(%,$mod,_prefix_chan)),\E,\E\\E\Q) $+ \E).*/Si):#: {
  ...
  if (%action) {
    hadd -mu6 $upper($mod) $+ _FLOOD %cn 1
    if (!$2) { %action ( $++ $wmm_bold($nick) $++ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(46)) $+ >) - ( $++ $lang(39) $wmm_bold($1 $wmm_under($gettok($horos,$rand(1,12),32))) $+ ) | return }
    if (!$istok($horos,$2,32)) { %action ( $++ $wmm_bold($nick) $++ ): $lang(43) $wmm_bold($horos) - ( $++ $lang(39) $wmm_bold($1 $wmm_under($gettok($horos,$rand(1,12),32))) $+ ) | return }
...
alias whs_ver { return $right($gettok($read($script,n,6),3,32),3) }
alias whs_crdate { return $remove($gettok($read($script,n,6),5,32),$chr(40),$chr(41)) }

alias -l mod { return whs }
alias -l command { return horo }
...
alias whs_horoscope_search {
  ...
  jsonopen -ud %v http://sandipbgt.com/theastrologer/api/horoscope/ $+ $4 $+ /today
  ...
  if ($istok(% [ $+ [ $mod ] $+ ] _show,description,32)) {
    var %desc = $json(%v,horoscope).value
    if (%desc) && ($len(%desc) >= 320) { var %desc = $left(%desc,$calc(320 -3)) $++ ... }
  }
  if ($istok(% [ $+ [ $mod ] $+ ] _show,link,32)) {
    var %url = https://new.theastrologer.com/ $+ $4
    if (% [ $+ [ $mod ] $+ ] _tiny) { var %url = $wmm_tinycom(%url) }
  }

  var %msg = 0,6 $+ $gettok($addon,1,45) $+ -0,14 $+ $gettok($addon,2,45) $+ : $wmm_bel $iif(%desc,$wmm_bold($lang(44)) $+ :14 %desc $+ ) $iif(%url,$wmm_sep $wmm_bold($lang(45)) $+ :12 $wmm_under(%url) $+ )
...
#whs_menu_status end
#whs_menu_channel off
menu channel { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#whs_menu_channel end
#whs_menu_nicklist off
menu nicklist { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#whs_menu_nicklist end
#whs_menu_query off
menu query { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
