...
dialog -l wcy_sets {
  ...
  check "Show the 'Value' information", 31, 160 38 164 10, tab 13
  ...
  text "Supported Countries", 53, 150 50 125 50, tab 13 center
...
ON *:DIALOG:wcy_sets:*:*: {
  if ($devent == init) {
    ...
    if ($istok(% [ $+ [ $mod ] $+ ] _show,value,32)) { did -c $dname 31 }
    ...
    var %z = 1
    while (%z <= $numtok($nations,44)) { did -a $dname 538 $gettok($nations,%z,44) | did -a $dname 539 $gettok($dnations,%z,44) | inc %z }
    did -e $dname 538,539
  ...
  if ($devent == close) {
    .timer[ $+ $mod $+ _*] off
    if (!% [ $+ [ $mod ] $+ ] _show) { set % $+ $mod $+ _show from value  }
...
ON *:LOAD: { 
  ...
  if (% [ $+ [ $mod ] $+ ] _show == $null) { set % $+ $mod $+ _show from to value }
  hfree -w  $mod $+ _*
  .signal -n wmm_close
  return
  :error | wmm_werror $addon $scriptline $error | reseterror
...
ON *:INPUT:#: {
  ...
  if (%action) {
    ...
    if (!$2) || (!$3) || (!$4) { .timer -h 1 0 %action ( $++ $wmm_bold($me) $++ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lower($lang(43))) $+ > < $+ $wmm_under($lower($lang(44))) $+ > < $+ $wmm_under($lower($lang(46))) $+ >) - ( $++ $lang(39) $wmm_bold($1 $wmm_under(USD) $wmm_under(EUR) $wmm_under(100)) $+ ) | return }
    if (!$istok($nations,$2,44)) { .timer -h 1 0 %action ( $++ $wmm_bold($me) $++ ): $lang(45) - $lang(20) $qt($wmm_bold($2)) $lang(50) | %action ( $++ $wmm_bold($nick) $++ ): $lang(51) $wmm_bold($nations) | return }
    if (!$istok($nations,$3,44)) { .timer -h 1 0 %action ( $++ $wmm_bold($me) $++ ): $lang(45) - $lang(20) $qt($wmm_bold($3)) $lang(50) | %action ( $++ $wmm_bold($nick) $++ ): $lang(51) $wmm_bold($nations) | return }
    if ($2 == $3) { .timer -h 1 0 %action ( $++ $wmm_bold($me) $++ ): $lang(45) - $lang(52) - ( $++ $lang(39) $wmm_bold($1 $wmm_under(USD) $wmm_under(EUR) $wmm_under(100)) $+ ) | return }
    if ($4 !isnum) { .timer -h 1 0 %action ( $++ $wmm_bold($me) $++ ): $lang(45) - $lang(57) - ( $++ $lang(39) $wmm_bold($1 $wmm_under(USD) $wmm_under(EUR) $wmm_under(100)) $+ ) | return }
...
ON $*:TEXT:$(/^(\Q $+ $replacecs($evalnext($+(%,$mod,_prefix_nick)),\E,\E\\E\Q) $+ \E|\Q $+ $replacecs($evalnext($+(%,$mod,_prefix_chan)),\E,\E\\E\Q) $+ \E).*/Si):#: {
  ...
  if (%action) {
    hadd -mu6 $upper($mod) $+ _FLOOD %cn 1
    if (!$2) || (!$3) || (!$4) { %action ( $++ $wmm_bold($nick) $++ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lower($lang(43))) $+ > < $+ $wmm_under($lower($lang(44))) $+ > < $+ $wmm_under($lower($lang(46))) $+ >) - ( $++ $lang(39) $wmm_bold($1 $wmm_under(USD) $wmm_under(EUR) $wmm_under(100)) $+ ) | return }
    if (!$istok($nations,$2,44)) { %action ( $++ $wmm_bold($nick) $++ ): $lang(45) - $lang(20) $qt($wmm_bold($2)) $lang(50) | %action ( $++ $wmm_bold($nick) $++ ): $lang(51) $wmm_bold($nations) | return }
    if (!$istok($nations,$3,44)) { %action ( $++ $wmm_bold($nick) $++ ): $lang(45) - $lang(20) $qt($wmm_bold($3)) $lang(50) | %action ( $++ $wmm_bold($nick) $++ ): $lang(51) $wmm_bold($nations) | return }
    if ($2 == $3) { %action ( $++ $wmm_bold($nick) $++ ): $lang(45) - $lang(52) - ( $++ $lang(39) $wmm_bold($1 $wmm_under(USD) $wmm_under(EUR) $wmm_under(100)) $+ ) | return }
    if ($4 !isnum) { %action ( $++ $wmm_bold($nick) $++ ): $lang(45) - $lang(57) - ( $++ $lang(39) $wmm_bold($1 $wmm_under(USD) $wmm_under(EUR) $wmm_under(100)) $+ ) | return }
...
alias wcy_ver { return $right($gettok($read($script,n,6),3,32),3) }
alias wcy_crdate { return $remove($gettok($read($script,n,6),5,32),$chr(40),$chr(41)) }

alias -l mod { return wcy }
alias -l command { return currency }
alias -l manager { return $iif($isalias(wmm_ver),$wmm_ver,0) }
alias -l addon { return $gettok($read($script,n,5),3,32) }
alias -l dnations { return Australia (dollar),Bulgarian (lev),Brazilian (real),Canadian (dollar),Swiss (franc),Chinese (yuan renminbi),Czech (koruna),Danish (krone),European (euro),United Kindom (pound sterling),Hong Kong (dollar),Croatian (kuna),Hungarian (forint),Indonesian (rupiah),Israeli (new shekel),Indian (rupee),Japanese (yen),South Korean (won),Mexican (peso),Malaysian (ringgit),Norwegian (krone),New Zealand (dollar),Philippine (peso),Polish (zloty),Romanian (leu),Russian (ruble),Swedish (krona),Singapore (dollar),Thai (baht),Turkish (lira),United States (dollar),South African (rand) }
alias -l nations { return AUD,BGN,BRL,CAD,CHF,CNY,CZK,DKK,EUR,GBP,HKD,HRK,HUF,IDR,ILS,INR,JPY,KRW,MXN,MYR,NOK,NZD,PHP,PLN,RON,RUB,SEK,SGD,THB,TRY,USD,ZAR }
...
alias wcy_currency_converter {
  if (!$wmm_internet) || (!$1-) { return }
  if (msg isin $3) { var %output = $3 $2 }
  elseif (notice isin $3) { var %output = $3 $1 }
  if (!% [ $+ [ $mod ] $+ ] _show) { set % $+ $mod $+ _show from to value }

  var %v = currency_converter_ $+ $wmm_random

  jsonopen -Ud %v https://decapi.me/misc/currency?from= $+ $4 $+ &to= $+ $5 $+ &value= $+ $6-
  if ($jsonerror) { var %m = ( $+ $wmm_bold($1) $+ ): $lang(40) - ( $+ $lang(41) $wmm_bold($jsonerror) $+ ) | %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%m),%m) | return }

  var %r = $json(%v).httpbody

  if (%r == $null) { var %m = ( $+ $wmm_bold($1) $+ ): $lang(42) | %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%m),%m) | return }
  var %value = $gettok(%r,4,32)

  var %msg = 13,2 $+ $gettok($addon,1,45) $+ -0,14 $+ $gettok($addon,2,45) $+ : $wmm_bel $iif($istok(%wcy_show,from,32),$wmm_bold($lang(43)) $+ :4 $4 (4 $+ $wmm_italic($gettok($dnations,$findtok($nations,$4,1,44),44)) $+ ) $+ ) $iif($istok(%wcy_show,to,32),$wmm_sep $wmm_bold($lang(44)) $+ :3 $5 (3 $+ $wmm_italic($gettok($dnations,$findtok($nations,$5,1,44),44)) $+ ) $+ ) $iif($istok(%wcy_show,value,32),$wmm_sep $wmm_bold($lang(46)) $+ :10 %value $+ )
...
#wcy_menu_status end
#wcy_menu_channel off
menu channel { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#wcy_menu_channel end
#wcy_menu_nicklist off
menu nicklist { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#wcy_menu_nicklist end
#wcy_menu_query off
menu query { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
