...
dialog -l wyt_sets {
  ...
  check "Show the 'Dislikes' information", 33, 160 58 164 10, tab 13
  ...
  check "Show the 'Length' information", 39, 4 109 155 10, tab 13
  check "Show the 'Favorites' information", 40, 4 99 155 10, tab 13
  ...
  check "Display the module output messages", 620, 160 55 160 10, tab 45
  menu "Menu", 22
  item "Help", 23, 22
  item break, 501
  item "About", 24, 22
...
ON *:DIALOG:wyt_sets:*:*: {
  if ($devent == init) {
    ...
    did -ra $dname 33 $lang(47) $qt($lang(53)) $lang(48)
    ...
    did -ra $dname 39 $lang(47) $qt($lang(50)) $lang(48)
    did -ra $dname 40 $lang(47) $qt($lang(67)) $lang(48)
    ...
    if ($istok(% [ $+ [ $mod ] $+ ] _show,dislikes,32)) { did -c $dname 33 }
    ...
    if ($istok(% [ $+ [ $mod ] $+ ] _show,length,32)) { did -c $dname 39 }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,favorites,32)) { did -c $dname 40 }
...
ON *:LOAD: {   
  ...
  if (% [ $+ [ $mod ] $+ ] _show == $null) { set % $+ $mod $+ _show title length views likes dislikes comments link }
  hfree -w $mod $+ _*
  .signal -n wmm_close
  return
  :error | wmm_werror $addon $scriptline $error | reseterror
...
ON *:INPUT:#: {
  ...
  if (%action) {
    ...
    if (!$2) { .timer -h 1 0 %action ( $++ $wmm_bold($me) $++ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(46)) $+ >) - ( $++ $lang(39) $wmm_bold($1 $wmm_under(Eminem)) $+ ) | return }
...
ON $*:TEXT:$(/^(\Q $+ $replacecs($evalnext($+(%,$mod,_prefix_nick)),\E,\E\\E\Q) $+ \E|\Q $+ $replacecs($evalnext($+(%,$mod,_prefix_chan)),\E,\E\\E\Q) $+ \E).*/Si):#: {
  ...
  if (%action) {
    hadd -mu6 $upper($mod) $+ _FLOOD %cn 1
    if (!$2) { %action ( $++ $wmm_bold($nick) $++ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(46)) $+ >) - ( $++ $lang(39) $wmm_bold($1 $wmm_under(Eminem)) $+ ) | return }
...
alias wyt_ver { return $right($gettok($read($script,n,6),3,32),3) }
alias wyt_crdate { return $remove($gettok($read($script,n,6),5,32),$chr(40),$chr(41)) }

alias -l mod { return wyt }
alias -l command { return youtube }
...
alias -l get_duration { return $duration($duration($replacex($remove($1,P,T),W,wk $chr(32),D,days $chr(32),h,hrs $chr(32),M,mins $chr(32),S,secs)),3) }
...
alias wyt_youtube_search {
  ...
  var %v = youtube_ $+ $wmm_random

  jsonopen -ud %v https://www.googleapis.com/youtube/v3/search?key= $+ $wmm_apikey($addon) $+ &part=snippet&order=viewCount&q= $+ $wmm_urlencode($4-) $+ &type=video&maxResults= $+ $evalnext(% [ $+ [ $mod ] $+ ] _max_results)
  if ($jsonerror) { var %m = ( $+ $wmm_bold($1) $+ ): $lang(40) - ( $+ $lang(41) $wmm_bold($jsonerror) $+ ) | %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%m),%m) | return }

  var %tot = $json(%v,pageInfo,totalResults).value
  if (!%tot) { var %m = ( $+ $wmm_bold($1) $+ ): $lang(42) | %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%m),%m) | return }
  var %x = 0
  while (%x < % [ $+ [ $mod ] $+ ] _max_results) {
    var %id = $json(%v,items,%x,id,videoId).value
    if (%id) { var %ids = $addtok(%ids,%id,32) }
    inc %x  
  }
  if (!%ids) { var %m = ( $+ $wmm_bold($1) $+ ): $lang(42) | %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%m),%m) | return }
  var %i = 1
  while (%i <= $numtok(%ids,32)) {
    var %vv = youtube_ids_ $+ $wmm_random
    var %id = $gettok(%ids,%i,32)
    if (!%id) { goto next }
    var %total = $calc(%total +1)

    jsonopen -ud %vv https://www.googleapis.com/youtube/v3/videos?id= $+ %id $+ &key= $+ $wmm_apikey($addon) $+ &part=snippet,contentDetails,statistics,topicDetails
    if ($jsonerror) { var %m = ( $+ $wmm_bold($1) $+ ): $lang(40) - ( $+ $lang(41) $wmm_bold($jsonerror) $+ ) | %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%m),%m) | return }

    if ($istok(% [ $+ [ $mod ] $+ ] _show,title,32)) { 
      var %title = $wmm_html2asc($wmm_fixtab($json(%vv,items,0,snippet,title).value))
    ...
    if ($istok(% [ $+ [ $mod ] $+ ] _show,description,32)) { 
      var %desc = $wmm_html2asc($wmm_fixtab($json(%vv,items,0,snippet,description).value))
    ...
    if ($istok(% [ $+ [ $mod ] $+ ] _show,uploaded_on,32)) { 
      var %uploaded = $json(%vv,items,0,snippet,publishedAt).value
    ...
    if ($istok(% [ $+ [ $mod ] $+ ] _show,uploaded_by,32)) { 
      var %uploader = $gettok($json(%vv,items,0,snippet,channelTitle).value,1,45)
      if (!%uploader) { var %uploader = $lang(59) }
    }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,resolution,32)) { 
      var %dimension = $upper($json(%vv,items,0,contentDetails,dimension).value)
      var %definition = $upper($json(%vv,items,0,contentDetails,definition).value)
      if (%dimension) { var %opt = $addtok(%opt,%dimension,32) }
      if (%definition) { var %opt = $addtok(%opt,%definition,32) }
      if (!%opt) { var %opt = $lang(59) }
    }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,length,32)) { 
      var %duration = $json(%vv,items,0,contentDetails,duration).value
      if (%duration) { var %duration = $get_duration(%duration) }
      if (!%duration) { var %duration = 00:00:00 }
    }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,views,32)) { var %views = $json(%vv,items,0,statistics,viewCount).value }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,likes,32)) { var %likes = $json(%vv,items,0,statistics,likeCount).value }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,dislikes,32)) { var %dislikes = $json(%vv,items,0,statistics,dislikeCount).value }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,favorites,32)) { var %favorites = $json(%vv,items,0,statistics,favoriteCount).value }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,comments,32)) { var %comments = $json(%vv,items,0,statistics,commentCount).value }
    ...
    if (% [ $+ [ $mod ] $+ ] _show_empty) { 
      var %msg = 1,0You0,4Tube-0,14 $+ $gettok($addon,2,45) $+ : $wmm_bel $iif(%title !== $null,$wmm_bold($lang(49)) $+ :6 %title $+ ) $iif(%desc !== $null,$wmm_sep $wmm_bold($lang(65)) $+ :14 %desc $+ ) $iif(%duration !== $null,$wmm_sep $wmm_bold($lang(50)) $+ :10 %duration $+ ) $iif(%opt !== $null,$wmm_sep $wmm_bold($lang(54)) $+ :13 %opt $+ ) $iif(%views !== $null,$wmm_sep $wmm_bold($iif(%views == 1,$lang(43),$lang(51))) $+ :3 $gettok($bytes(%views,bd),1,46) $+ ) $iif(%favorites !== $null,$wmm_sep $wmm_bold($iif(%favorites == 1,$lang(66),$lang(67))) $+ :5 $gettok($bytes(%favorites,bd),1,46) $+ ) $iif(%likes !== $null,$wmm_sep $wmm_bold($iif(%likes == 1,$lang(44),$lang(52))) $+ :9 $gettok($bytes(%likes,bd),1,46) $+ ) $iif(%dislikes !== $null,$wmm_sep $wmm_bold($iif(%dislikes == 1,$lang(45),$lang(53))) $+ :4 $gettok($bytes(%dislikes,bd),1,46) $+ ) $iif(%comments !== $null,$wmm_sep $wmm_bold($lang(55)) $+ :12 $gettok($bytes(%comments,bd),1,46) $+ ) $iif(%uploader !== $null,$wmm_sep $wmm_bold($lang(57)) $+ :2 %uploader $+ ) $iif(%uploaded !== $null,$wmm_sep $wmm_bold($lang(56)) $+ :7 %uploaded $+ ) $iif(%url !== $null,$wmm_sep $wmm_bold($lang(58)) $+ :12 $wmm_under(%url) $+ )
      %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%msg),%msg)
    }
    elseif (!% [ $+ [ $mod ] $+ ] _show_empty) { 
      var %msg = 1,0You0,4Tube-0,14 $+ $gettok($addon,2,45) $+ : $wmm_bel $iif(%title,$wmm_bold($lang(49)) $+ :6 %title $+ ) $iif(%desc,$wmm_sep $wmm_bold($lang(65)) $+ :14 %desc $+ ) $iif(%duration,$wmm_sep $wmm_bold($lang(50)) $+ :10 %duration $+ ) $iif(%opt,$wmm_sep $wmm_bold($lang(54)) $+ :13 %opt $+ ) $iif(%views,$wmm_sep $wmm_bold($iif(%views == 1,$lang(43),$lang(51))) $+ :3 $gettok($bytes(%views,bd),1,46) $+ ) $iif(%favorites,$wmm_sep $wmm_bold($iif(%favorites == 1,$lang(66),$lang(67))) $+ :5 $gettok($bytes(%favorites,bd),1,46) $+ ) $iif(%likes,$wmm_sep $wmm_bold($iif(%likes == 1,$lang(44),$lang(52))) $+ :9 $gettok($bytes(%likes,bd),1,46) $+ ) $iif(%dislikes,$wmm_sep $wmm_bold($iif(%dislikes == 1,$lang(45),$lang(53))) $+ :4 $gettok($bytes(%dislikes,bd),1,46) $+ ) $iif(%comments,$wmm_sep $wmm_bold($lang(55)) $+ :12 $gettok($bytes(%comments,bd),1,46) $+ ) $iif(%uploader,$wmm_sep $wmm_bold($lang(57)) $+ :2 %uploader $+ ) $iif(%uploaded,$wmm_sep $wmm_bold($lang(56)) $+ :7 %uploaded $+ ) $iif(%url,$wmm_sep $wmm_bold($lang(58)) $+ :12 $wmm_under(%url) $+ )
      %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%msg),%msg)
    }
...
#wyt_menu_status end
#wyt_menu_channel off
menu channel { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#wyt_menu_channel end
#wyt_menu_nicklist off
menu nicklist { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#wyt_menu_nicklist end
#wyt_menu_query off
menu query { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
