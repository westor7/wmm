...
ON *:INPUT:#: {
  ...
  if (%action) {
    ...
    if (!$2) { .timer -h 1 0 %action ( $++ $wmm_bold($me) $++ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(43)) $+ >) - ( $++ $lang(39) $wmm_bold($1 $wmm_under($serverip)) $+ ) | return }
    if (!$wmm_check_ipv4($2)) { .timer -h 1 0 %action ( $++ $wmm_bold($me) $++ ): $lang(46) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(43)) $+ >) - ( $++ $lang(39) $wmm_bold($1 $wmm_under($serverip)) $+ ) | return }
...
ON $*:TEXT:$(/^(\Q $+ $replacecs($evalnext($+(%,$mod,_prefix_nick)),\E,\E\\E\Q) $+ \E|\Q $+ $replacecs($evalnext($+(%,$mod,_prefix_chan)),\E,\E\\E\Q) $+ \E).*/Si):#: {
  ...
  if (%action) {
    hadd -mu6 $upper($mod) $+ _FLOOD %cn 1
    if (!$2) { %action ( $++ $wmm_bold($nick) $++ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(43)) $+ >) - ( $++ $lang(39) $wmm_bold($1 $wmm_under($serverip)) $+ ) | return }
    if (!$wmm_check_ipv4($2)) { %action ( $++ $wmm_bold($nick) $++ ): $lang(46) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lang(43)) $+ >) - ( $++ $lang(39) $wmm_bold($1 $wmm_under($serverip)) $+ ) | return }
...
alias wiv_ver { return $right($gettok($read($script,n,6),3,32),3) }
alias wiv_crdate { return $remove($gettok($read($script,n,6),5,32),$chr(40),$chr(41)) }

alias -l mod { return wiv }
alias -l command { return ipv6 }
...
alias wiv_ipv6_search {
  if (!$wmm_internet) || (!$1-) { return }
  if (msg isin $3) { var %output = $3 $2 }
  elseif (notice isin $3) { var %output = $3 $1 }
  var %v = ipv6_checker_ $+ $wmm_random

  jsonopen -dU %v https://www.ultratools.com/tools/ipv4toipv6Result?address= $+ $4
  if ($jsonerror) { var %m = ( $+ $wmm_bold($1) $+ ): $lang(25) $+ , $lang(47) | %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%m),%m) | return }
  ...
  var %rr = $json(%v).httpbodytobvar
  var %r = $wmm_fixtab($bvar(%rr,18000-19000).text)

  var %ip = $gettok($gettok(%r,2,62),1,60)

  ; var %fn = $calc($findtok(%r,</span>,1,32) -2) - $calc($findtok(%r,</span>,1,32) -1)
  ; var %ip = $wmm_nohtml($gettok(%r,%fn,32))

  if (!%ip) { var %m = ( $+ $wmm_bold($1) $+ ): $lang(42) | %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%m),%m) | return }
  if (!$wmm_check_ipv6(%ip)) { var %m = ( $+ $wmm_bold($1) $+ ): $lang(25) $+ , $lang(45) $wmm_under($4) $lang(51) | %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%m),%m) | return }

  var %m = ( $+ $wmm_bold($1) $+ ): $lang(45) $wmm_under($4) $lang(52) $wmm_under(%ip)
  %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%m),%m)
...
#wiv_menu_status end
#wiv_menu_channel off
menu channel { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#wiv_menu_channel end
#wiv_menu_nicklist off
menu nicklist { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#wiv_menu_nicklist end
#wiv_menu_query off
menu query { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
