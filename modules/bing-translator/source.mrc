...
dialog -l wbt_sets {
  ...
  check "Show the 'From' information", 29, 160 18 164 10, tab 13
  check "Show the 'To' information", 30, 160 28 164 10, tab 13
  check "Show the 'Text' information", 31, 160 38 164 10, tab 13
...
ON *:DIALOG:wbt_sets:*:*: {
  if ($devent == init) {
    ...
    if ($istok(% [ $+ [ $mod ] $+ ] _show,from,32)) { did -c $dname 29 }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,to,32)) { did -c $dname 30 }
    if ($istok(% [ $+ [ $mod ] $+ ] _show,text,32)) { did -c $dname 31 }
  ...
  if ($devent == close) {
    .timer[ $+ $mod $+ _*] off
    if (!% [ $+ [ $mod ] $+ ] _show) { set % $+ $mod $+ _show name link  }
...
ON *:LOAD: { 
  ...
  if (% [ $+ [ $mod ] $+ ] _show == $null) { set % $+ $mod $+ _show from to text }
  hfree -w  $mod $+ _*
  .signal -n wmm_close
  return
  :error | wmm_werror $addon $scriptline $error | reseterror
...
ON *:INPUT:#: {
  ...
  if (%action) {
    ...
    if (!$2) || (!$3) || (!$4) { .timer -h 1 0 %action ( $++ $wmm_bold($me) $++ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lower($lang(43))) $+ > < $+ $wmm_under($lower($lang(44))) $+ > < $+ $wmm_under($lower($lang(46))) $+ >) - ( $++ $lang(39) $wmm_bold($1 $wmm_under(en) $wmm_under(el) $wmm_under(Hello)) $+ ) | return }
    if (!$istok($langs,$2,44)) { .timer -h 1 0 %action ( $++ $wmm_bold($me) $++ ): $lang(45) - $lang(20) $qt($wmm_bold($2)) $lang(50) | %action ( $++ $wmm_bold($nick) $++ ): $lang(51) $wmm_bold($langs) | return }
    if (!$istok($langs,$3,44)) { .timer -h 1 0 %action ( $++ $wmm_bold($me) $++ ): $lang(45) - $lang(20) $qt($wmm_bold($3)) $lang(50) | %action ( $++ $wmm_bold($nick) $++ ): $lang(51) $wmm_bold($langs) | return }
    if ($2 == $3) { .timer -h 1 0 %action ( $++ $wmm_bold($me) $++ ): $lang(45) - $lang(52) - ( $++ $lang(39) $wmm_bold($1 $wmm_under(en) $wmm_under(el) $wmm_under(Hello)) $+ ) | return }
...
ON $*:TEXT:$(/^(\Q $+ $replacecs($evalnext($+(%,$mod,_prefix_nick)),\E,\E\\E\Q) $+ \E|\Q $+ $replacecs($evalnext($+(%,$mod,_prefix_chan)),\E,\E\\E\Q) $+ \E).*/Si):#: {
  ...
  if (%action) {
    hadd -mu6 $upper($mod) $+ _FLOOD %cn 1
    if (!$2) || (!$3) || (!$4) { %action ( $++ $wmm_bold($nick) $++ ): $lang(37) - $lang(38) $wmm_bold($1 < $+ $wmm_under($lower($lang(43))) $+ > < $+ $wmm_under($lower($lang(44))) $+ > < $+ $wmm_under($lower($lang(46))) $+ >) - ( $++ $lang(39) $wmm_bold($1 $wmm_under(en) $wmm_under(el) $wmm_under(Hello)) $+ ) | return }
    if (!$istok($langs,$2,44)) { %action ( $++ $wmm_bold($nick) $++ ): $lang(45) - $lang(20) $qt($wmm_bold($2)) $lang(50) | %action ( $++ $wmm_bold($nick) $++ ): $lang(51) $wmm_bold($langs) | return }
    if (!$istok($langs,$3,44)) { %action ( $++ $wmm_bold($nick) $++ ): $lang(45) - $lang(20) $qt($wmm_bold($3)) $lang(50) | %action ( $++ $wmm_bold($nick) $++ ): $lang(51) $wmm_bold($langs) | return }
    if ($2 == $3) { %action ( $++ $wmm_bold($nick) $++ ): $lang(45) - $lang(52) - ( $++ $lang(39) $wmm_bold($1 $wmm_under(en) $wmm_under(el) $wmm_under(Hello)) $+ ) | return }
...
alias wbt_ver { return $right($gettok($read($script,n,6),3,32),3) }
alias wbt_crdate { return $remove($gettok($read($script,n,6),5,32),$chr(40),$chr(41)) }

alias -l mod { return wbt }
alias -l command { return translate }
alias -l manager { return $iif($isalias(wmm_ver),$wmm_ver,0) }
alias -l addon { return $gettok($read($script,n,5),3,32) }
alias -l dlangs { return Afrikaans,Albanian,Amharic,Antigua and Barbuda Creole English,Arabic,Aragonese,Armenian,Asturian,Azerbaijani,Bahamas Creole English,Bajan,Balkan Gipsy,Basque,Bemba,Bengali,Bielarus,Bislama,Bosnian,Breton,Bulgarian,Burmese,Catalan,Catalan Valencia,Cebuano,Chamorro,Chinese,Classical Greek,Comorian Ngazidja,Coptic,Crioulo Upper Guinea,Croatian,Czech,Danish,Dutch,Dzongkha,English,Esperanto,Estonian,Fanagalo,Faroese,Filipino,Finnish,French,Galician,Georgian,German,Greek,Grenadian Creole English,Gujarati,Guyanese Creole English,Haitian Creole French,Hausa,Hawaiian,Hebrew,Hindi,Hungarian,Icelandic,Indonesian,Inuktitut Greenlandic,Irish Gaelic,Italian,Jamaican Creole English,Japanese,Javanese,Kabuverdianu,Kabylian,Kannada,Kazakh,Khmer,Kinyarwanda,Kirundi,Klingon,Korean,Kurdish,Kyrgyz,Lao,Latin,Latvian,Lithuanian,Luxembourgish,Macedonian,Malagasy,Malay,Malayalam,Maldivian,Maltese,Manx Gaelic,Maori,Marathi,Marshallese,Mende,Mongolian,Montenegrin,Morisyen,Ndebele,Nepali,Niuean,Northern Sesotho,Norwegian,Norwegian Bokmal,Norwegian Nynorsk,Nyanja,Occitan,Pakistani,Palauan,Panjabi,Papiamentu,Pashto,Persian,Pijin,Polish,Portuguese,Potawatomi,Quechua,Romanian,Russian,Saint Lucian Creole French,Samoan,Sango,Scots Gaelic,Serbian,Serbo-Croatian,Seselwa Creole French,Shona,Sinhala,Slovak,Slovenian,Somali,Sotho Southern,Spanish,Spanish Colombian,Sranan Tongo,Swahili,Swedish,Syriac Aramaic,Tagalog,Tajik,Tamashek,Tamil,Tatar,Telugu,Tetum,Thai,Tibetan,Tigrinya,Tok Pisin,Tokelauan,Tongan,Tsonga,Tswana,Turkish,Turkmen,Tuvaluan,Ukrainian,Uma,Uzbek,Vietnamese,Vincentian Creole English,Virgin Islands Creole English,Wallisian,Welsh,Wolof,Xhosa,Yiddish,Yoruba,Zulu }
alias -l langs { return af,sq,am,aig,ar,an,hy,ast,az,bah,bjs,rm,eu,bem,bn,be,bi,bs,br,bg,my,ca,cav,cb,ch,zh,grc,zdj,cop,pov,hr,cs,da,nl,dz,en,eo,et,fn,fo,fil,fi,fr,gl,ka,de,el,gcl,gu,gyn,ht,ha,haw,he,hi,hu,is,id,kl,ga,it,jam,ja,jw,kea,kab,kn,kk,km,rw,rn,tlh,ko,ku,ky,lo,la,lv,lt,lb,mk,mg,ms,ml,dv,mt,gv,mi,mr,mh,men,mn,srp,mfe,nd,ne,niu,nso,no,nb,nn,ny,oc,ur,pau,pa,x1,ps,fa,pis,pl,pt,pot,qu,ro,ru,acf,sm,sg,gd,sr,sh,crs,sn,si,sk,sl,so,st,es,col,srn,sw,sv,syc,tl,tg,tmh,ta,tt,te,tet,th,bo,ti,tpi,tkl,to,ts,tn,tr,tk,tvl,uk,ppk,uz,vi,svc,vic,wls,cy,wo,xh,yi,yo,zu }
...
alias wbt_bing_translator {
  if (!$wmm_internet) || (!$1-) { return }
  if (msg isin $3) { var %output = $3 $2 }
  elseif (notice isin $3) { var %output = $3 $1 }
  if (!% [ $+ [ $mod ] $+ ] _show) { set % $+ $mod $+ _show from to text }

  var %v = bing_translator_ $+ $wmm_random

  jsonopen -ud %v https://api.mymemory.translated.net/get?q= $+ $wmm_urlencode($6-) $+ &langpair= $+ $4 $+ $chr(124) $+ $5 $+ &de= $+ $wbt_owner $+ 7@gmail.com
  if ($jsonerror) { var %m = ( $+ $wmm_bold($1) $+ ): $lang(40) - ( $+ $lang(41) $wmm_bold($jsonerror) $+ ) | %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%m),%m) | return }

  var %match = $json(%v,responseData,match).value
  if (%match == $null) { var %m = ( $+ $wmm_bold($1) $+ ): $lang(42) | %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%m),%m) | return }

  var %text_out = $json(%v,responseData,translatedText).value
  if (!%text_out) { var %m = ( $+ $wmm_bold($1) $+ ): $lang(42) | %output $iif(% [ $+ [ $mod ] $+ ] _strip,$strip(%m),%m) | return }

  var %msg = 2,7 $+ $gettok($addon,1,45) $+ -0,14 $+ $gettok($addon,2,45) $+ : $wmm_bel $iif($istok(%wbt_show,from,32),$wmm_bold($lang(43)) $+ :4 $4 (4 $+ $wmm_italic($gettok($dlangs,$findtok($langs,$4,1,44),44)) $+ ) $+ ) $iif($istok(%wbt_show,to,32),$wmm_sep $wmm_bold($lang(44)) $+ :3 $5 (3 $+ $wmm_italic($gettok($dlangs,$findtok($langs,$5,1,44),44)) $+ ) $+ ) $iif($istok(%wbt_show,text,32),$wmm_sep $wmm_bold($lang(46)) $+ :10 %text_out $+ )
...
#wbt_menu_status end
#wbt_menu_channel off
menu channel { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#wbt_menu_channel end
#wbt_menu_nicklist off
menu nicklist { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
  -
}
#wbt_menu_nicklist end
#wbt_menu_query off
menu query { 
  -
  $iif($isalias(wmm_isadi) && $wmm_isadi && $file($scriptdir $+ $mod $+ _main.ico),$menuicon($scriptdir $+ $mod $+ _main.ico)) $iif($dialog( [ $+ [ $mod ] $+ ] _sets),$style(1)) $iif($isalias(wmm_qd),$wmm_qd($addon v $+ $ [ $+ [ $mod ] $+ ] _ver - $iif($lang(4),$v1,Settings) $+ ),* $addon v $+ $ [ $+ [ $mod ] $+ ] _ver - Settings *): $+ $mod $+ _sets
